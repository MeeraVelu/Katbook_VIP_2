...
from .utils import log, free_vram, managed_model
...
def mean_rms_db(wav_path: Path) -> float:
    ...
    vals = [float(l.split("=")[-1]) for l in cp.stderr.splitlines()
            if "RMS_level" in l and "=" in l and "inf" not in l.split("=")[-1].lower()]
...
def transcribe(wav_path: Path, *, model_name: str, device: str,
    ...
    with managed_model("Whisper") as keep:
        ...
        seg_iter, info = whisper.transcribe(
            str(wav_path), beam_size=beam_size, vad_filter=True,
            condition_on_previous_text=False, word_timestamps=False)
        transcript = [{"start": round(s.start, 2), "end": round(s.end, 2),
    ...
    log(f"whisper: lang={info.language} ({info.language_probability:.2f}) "
    ...
    return {"transcript": transcript, "language": info.language,
            ...
            "full_text": full_text, "speech_sec": round(speech_sec, 1)}
...
def audio_features(wav_path: Path, duration: float, full_text: str) -> dict:
    ...
    try:
        ...
        return {"rms_energy": None, "silence_ratio": None,
                "words_per_minute": round(len(full_text.split()) /
                                          max(duration / 60, 1e-6), 1),
