...
from .storage import find_canonical_by_hash, store, store_duplicate
from .utils import capture_runtime, load_checkpoint, log, save_checkpoint, timer
...
def process_one_video(
    video_path: str, cfg: dict, *, embedder, nlp, engine=None, device: str = "cuda", progress=None
) -> dict:
    """Run every stage for one video.

    ``progress`` (optional) is ``callable(stage: str, info: dict)`` invoked after
    each stage completes; the Celery worker uses it to publish live job state
    (current stage + accumulated timings) so ``GET /jobs/{id}`` reflects progress.
    """
    t_all = time.time()
    ...
    P: dict = {
        ...
        "stage_timings": {},
        "status": "processing",
        # provenance of WHERE this ran, captured here so a downstream
        # reader reports the true processing env, not its own.
        "runtime": capture_runtime(cfg),
    }
    bind(video_id=P["video_id"][:8])
    report("start", {})
    ...
    P["file_size_bytes"] = ingest.file_size_bytes(video_path)
    if engine is not None and cfg.get("DEDUP_BY_HASH", True):
        ...
        if canonical:
            store_duplicate(
                ...
                P["content_hash"],
                canonical,
                file_size=P["file_size_bytes"],
            )
            log(f"DUPLICATE of {canonical[:8]} — skipped (no processing)")
            report("duplicate", {"duplicate_of": canonical[:8]})
            return {
                "video": Path(video_path).name,
                "video_id": P["video_id"][:8],
                "duplicate_of": canonical[:8],
                "status": "duplicate",
            }
    ...
    report("ingest_audio", {"stage_timings": P["stage_timings"]})

    # ---- Stage 1.5: speech detection -> ROUTE ----
    has_speech, rms_db = (
        audio.detect_speech(audio_path, cfg["SILENCE_DB"]) if wav_ok else (False, -99.0)
    )
    ...
    if has_speech:
        ...
        if ckpt and ckpt.get("transcript"):
            ...
            save_checkpoint(
                cfg["WORK_DIR"], cfg["CHECKPOINT_SUBDIR"], P["video_id"], transcribe_out
            )

    route = decide(
        has_speech=has_speech, mean_rms_db=rms_db, speech_sec=transcribe_out["speech_sec"], cfg=cfg
    )
    P.update(
        {
            "transcript": transcribe_out["transcript"],
            "language": transcribe_out["language"],
            "has_speech": route.has_speech,
            "tagging_path": route.path,
            "audio_features": {
                "mean_rms_db": rms_db,
    ...
    report(
        "route",
        {"path": route.path, "has_speech": route.has_speech, "stage_timings": P["stage_timings"]},
    )
    ...
    with timer(P["stage_timings"], "extract_frames"):
        ...
        P["frame_analyses"] = ingest.extract_frames_at(
            video_path, times, frames_dir, hwaccel=cfg.get("FFMPEG_HWACCEL", "none")
        )
    log(f"frames: {len(P['frame_analyses'])} (cap {route.max_frames}, path {route.path})")
    report(
        "extract_frames", {"frames": len(P["frame_analyses"]), "stage_timings": P["stage_timings"]}
    )
    ...
    if route.path == VOICE and wav_ok and cfg.get("RUN_AUDIO_FEATURES", False):
        with timer(P["stage_timings"], "audio_features"):
            P["audio_features"].update(
                audio.audio_features(audio_path, P["duration"], transcribe_out["full_text"])
            )
    ...
    with timer(P["stage_timings"], "visual"):
        analyze_frames(P["frame_analyses"], cfg, device, is_silent=(route.path == SILENT))
    report("visual", {"stage_timings": P["stage_timings"]})
    ...
    with timer(P["stage_timings"], "segmentation"):
        ...
        P["segments"] = segment.attach_signals(segs, P["transcript"], P["frame_analyses"])
    log(f"segments: {len(P['segments'])}")
    report("segmentation", {"segments": len(P["segments"]), "stage_timings": P["stage_timings"]})
    ...
    report("llm", {"stage_timings": P["stage_timings"]})
