...
def load_config(
    overrides: dict[str, Any] | None = None, *, apply_tier: bool = False
) -> dict[str, Any]:
    """Merge Settings(env) + optional GPU tier + chosen PROFILE into one flat dict.

    Precedence (highest wins): caller ``overrides`` -> explicit env overrides
    (e.g. ``KVIP_EMBED_MODEL``) -> ``apply_tier`` GPU-tier knobs -> PROFILE defaults.

    ``apply_tier`` is opt-in: the Celery worker enables it so the pipeline adapts
    to the detected GPU (see :mod:`pipeline.gpu_profile`); the batch CLI and the
    CPU smoke test leave it off and get plain profile behavior.
    """
    ...
    # GPU-tier layer (below explicit env/overrides, above profile defaults).
    if apply_tier:
        from .gpu_profile import detect_tier, tier_config

        for k, v in tier_config(detect_tier()).items():
            cfg.setdefault(k, v)
    ...
    cfg["PROFILE"] = profile
    # profile fills any gap the env/overrides/tier did not already set
    for k, v in PROFILES[profile].items():
        cfg.setdefault(k, v)

    # OCR languages: explicit env > tier > final fallback (English always works).
    cfg.setdefault("OCR_LANGS", ["en"])
